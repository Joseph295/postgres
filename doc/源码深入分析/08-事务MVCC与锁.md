...
10. 全章不变式清单（8.10）、设计史（8.11）与 InnoDB 对照总表（8.12）。

阅读建议：8.3/8.4（快照与可见性）与 8.6（锁）是两条可独立阅读的主线，8.9（SSI）同时依赖两条主线；赶时间可先读各节"所以然"与小结，回头再啃走读。
...
- **SQL 标准四隔离级别与三种读现象**（后文以缩写指代）：

| 级别 | 脏读 | 不可重复读 | 幻读 | PG 实际行为 |
|---|---|---|---|---|
| READ UNCOMMITTED | 标准允许 | 允许 | 允许 | = RC（PG 不做脏读） |
| READ COMMITTED (RC) | 禁止 | 允许 | 允许 | 每语句新快照 |
| REPEATABLE READ (RR) | 禁止 | 禁止 | 标准允许 | 快照隔离：幻读也没有，但有 write skew（8.9） |
| SERIALIZABLE | 禁止 | 禁止 | 禁止 | SSI：等价某个串行序，代价是 40001 重试 |

### 开篇串场：一条 UPDATE 的并发之旅

先纵览一遍"会话 A 执行 `UPDATE acct SET bal = bal - 100 WHERE id = 7;`"在本章各机制间的完整路线，作为后续各节的地图（括号内为详述小节）：

1. 语句开始，`GetTransactionSnapshot()` 拍快照——持 ProcArrayLock 共享锁扫稠密数组，或凭 xactCompletionCount 复用上一张（8.3）；
2. 计划执行到 SeqScan/IndexScan，逐行调 `HeapTupleSatisfiesMVCC`：hint bit → 自己？→ 快照？→ CLOG，找到 id=7 当前对我可见的版本（8.4、8.5）；
3. 首次写操作触发 `AssignTransactionId()`：拿 XidGenLock 领真 XID、写进 ProcArray、对自己的 XID 加排他事务锁（8.2）；
4. 表级锁其实早在解析后就拿了：RowExclusiveLock，大概率走 fastpath 记在自家 PGPROC 槽位（8.6.3）；
5. `heap_update` 用 `HeapTupleSatisfiesUpdate` 给目标行定性（8.4.5）：若 TM_BeingModified——有人正锁/改着——先拿 tuple 锁排队票，再对其 XID 的事务锁做共享申请，进 `ProcSleep` 睡 latch 并武装 deadlock_timeout（8.6.4）；若睡满 1 秒，`CheckDeadLock` 锁 16 分区查等待图（8.8）；
6. 等到手后（RC 可能还要过 EPQ 重验 WHERE，8.3.6），写入：旧版本 t_xmax=我的 XID、新版本入页、ctid 链接、写 WAL；若行上原有共享锁持有者，xmax 合成 multixact（8.7）；
7. 若是 SERIALIZABLE：写之前 `CheckForSerializableConflictIn` 查这行/页/表上谁留过 SIREAD 字条，建 rw 边并当场检危险结构（8.9）；
8. COMMIT：`RecordTransactionCommit` 写 WAL+CLOG（持久性），`ProcArrayEndTransaction` 清 ProcArray（可见性），锁全释放，等我事务锁的行锁等待者集体醒来（8.1）。

一条最普通的 UPDATE 走完了本章全部九节。下面开始逐站拆解。
...
为什么要两层？因为 PG 中"一条不带 BEGIN 的 SQL"也是一个事务（隐式事务），而用户显式的 `BEGIN ... COMMIT` 块可以横跨多条命令、中途出错后还要停在"等待 ROLLBACK"的状态。高层状态机就是协调"命令边界"与"事务边界"错位的机构。低层 `TransState`（`xact.c:143`）只有六个值，描述事务本体的物理阶段：
...
TRANS_DEFAULT ──StartTransaction──> TRANS_START ──> TRANS_INPROGRESS
                                     ...
                                     CommitTransaction ─┘        └─ AbortTransaction
                                            ↓                           ↓
                                      TRANS_COMMIT ──> DEFAULT    TRANS_ABORT ──> DEFAULT
                                     （另有 TRANS_PREPARE 供两阶段提交）
```

两层的分工可以这样记：**TransState 回答"事务这台机器现在处于哪个物理档位"，TBlockState 回答"用户以为自己在事务的哪一步"**。二者常常不同步——比如事务块内出错后，TransState 已经走完 ABORT 回到可清理状态，TBlockState 却停在 TBLOCK_ABORT 等用户发落；又如收到 COMMIT 命令时 TBlockState 先变 TBLOCK_END，TransState 仍是 INPROGRESS，直到命令边界才真正走 TRANS_COMMIT。所有"PG 事务行为怪癖"几乎都能在这两层的错位里找到解释。
...
顺带一提：PL/pgSQL 的 `BEGIN...EXCEPTION` 块底层就是子事务，这也是"EXCEPTION 块很贵"的根源——每次进入都要 push 状态、可能分配子 XID、写 pg_subtrans。8.4.6 的 what-if 会推演它最恶劣的形态。

### 8.1.4 TransactionStateData：状态机的载体

上面反复出现的"事务状态"实体定义在 `xact.c:195`（节选，省略并行相关注释）：
...
- **两个状态字段并排**（state/blockState）：双层状态机同居一个结构，`CurrentTransactionState` 永远指向栈顶；顶层事务是静态分配的 `TopTransactionStateData`，子事务才动态 palloc——单语句路径零分配；
- **`childXids` 有序数组**：子事务提交后其 XID 归并进父层（保持 XID 排序，`TransactionIdIsCurrentTransactionId` 用二分查找），这就是 8.4.2 走读里"是不是我自己的事务"能纯本地判断的数据来源——**自己家的族谱自己存一份**，不必去查共享内存或 pg_subtrans；
- **`prevUser`/`prevSecContext`/`prevXactReadOnly`**：SAVEPOINT 回滚要恢复的不止数据——SECURITY DEFINER 函数切换的身份、SET LOCAL 改的 GUC（经 gucNestLevel 配合 GUC 栈）都要精确回退。事务状态栈本质上是**数据库世界的调用栈**，回滚 = 弹栈 + 逐项恢复副作用；
- **`didLogXid`**：本 XID 是否已随某条 WAL 记录写出——决定后续 WAL 记录是否还要携带顶层 XID（wal_level=logical 的备库需要靠它把子事务归组）。
...
- 函数尾部是并发正确性的核心。先看顶层事务的发布代码（`varsup.c:246-256`，原文）：
...
```
  ...
  其上的长注释（`varsup.c:212-244`）交代了全部并发契约：**新 XID 必须在释放 XidGenLock 之前写进 ProcArray 的 xids 数组**——"这保证任何比 latestCompletedXid 老的活跃 XID 一定出现在 ProcArray 里，这是 OldestXmin 追踪正确性的根本（见 transam/README）"；读方"对每个 proc 的 xid 只能取一次值，不能假设重复读能得到相同答案"（8.3.3 走读里的 `UINT32_ACCESS_ONCE` 就是兑现）；子事务路径则要求"先填数组元素、写屏障、再增 nxids 计数"，读方反向配 `pg_read_barrier`——否则读者可能读到"计数已加而元素未填"的中间态。注释还坦白了一个**允许存在的竞态窗口**：子 XID 写进 ProcArray 后、其父链写进 pg_subtrans 前，别人查这个子 XID 的状态会失败——但"任何人有理由查询它之前，父链必已就位"（快照此窗口期照样会把父 XID 收进去，事后推导仍正确）。这种"证明窗口无害"而非"消灭窗口"的手法，是共享内存编程里控制锁粒度的常用刀法。
...
```sql
-- 各库距回卷的年龄（超过 autovacuum_freeze_max_age=2 亿即进入强制 vacuum 区）
SELECT datname, age(datfrozenxid) FROM pg_database ORDER BY 2 DESC;
-- 定位拖后腿的表
SELECT relname, age(relfrozenxid) FROM pg_class
 WHERE relkind IN ('r','t','m') ORDER BY 2 DESC LIMIT 10;
-- 谁在钉住视界: 长事务 / 孤儿 prepared 事务 / 落后的复制槽
SELECT pid, age(backend_xmin), state FROM pg_stat_activity WHERE backend_xmin IS NOT NULL;
SELECT gid, prepared FROM pg_prepared_xacts;
SELECT slot_name, age(xmin) FROM pg_replication_slots;
```

三个"钉子"（长事务、遗忘的 prepared 事务、失联复制槽）任何一个都能让 autovacuum 空转——freeze 推不动 relfrozenxid，年龄持续上涨直到停库。回卷事故的根因几乎从来不是"vacuum 不够勤"，而是这三颗钉子之一。
...
### 8.2.6 what-if：如果防线全破、回卷真的发生了？

思想实验：假设四道防线全部失效（比如单用户模式里用危险手段强推了计数器），nextXid 越过了某张表里最老未冻结元组的"对面半圈"，会看到什么？

1. **数据"蒸发"而非报错**：老元组的 xmin（比如 XID=100，已提交多年）与新 nextXid（比如 100 + 2^31 + 1000）比较时，`(int32)` 差值变成正数——`TransactionIdPrecedes(100, cur)` 返回 false，100 被判定为"未来的事务"。可见性判断走到 [X3]/[X5] 类分支：这些行**静默不可见**。SELECT 返回空集、COUNT 掉到零，但没有任何错误——比崩溃更险恶，备份也可能默默备出空表；
2. **hint bit 的诡异残留**：这些行大多带着 `HEAP_XMIN_COMMITTED` hint。回看 8.4.2 [X6] 分支：hint 已提交仍要过 `XidInMVCCSnapshot`——xmin 被判"≥ xmax 即在跑"→ 不可见。也就是说 hint 救不了它们，**只有 frozen 位能豁免比较**（[X6] 的 `!HeapTupleHeaderXminFrozen` 短路）——这就是为什么 freeze 是唯一根治，而不是"多设点 hint"；
3. **CLOG 已被截断**：更早之前 `oldestXid` 推进时，老 XID 的 CLOG 页已物理删除。任何路径真去查 `TransactionIdDidCommit(100)` 会报 "could not access status of transaction"——实践中回卷事故的第一现场往往正是这条报错；
4. **部分可见的撕裂表**：同一张表里，冻结过的行照常可见、没冻结的老行消失——应用看到逻辑上不可能的状态（外键指向"不存在"的主键行）。
5. **恢复手段**：立刻停库；用未受损的备份+WAL 恢复到事故点前；若无备份，pg_resetwal 强改 epoch 可让部分数据重现，但一致性无保证——本质是灾难考古而非恢复。

这个推演的教学价值在于把 8.2 的所有零件串成因果链：模比较的半径（8.2.3）解释"为什么是静默消失"；hint/frozen 的区别（8.4）解释"为什么 freeze 不可替代"；CLOG 截断（8.5）解释"为什么还伴随报错"。真实世界里防线 3（xidStopLimit 停库）几乎总能兜住——著名的公开事故（如 Sentry 2015、Mailchimp/Joyent 等）都停在"被迫停库若干小时做紧急 VACUUM"，尚无防线全破的公开案例。
...
重构后的布局是**列式思维**：把"拍快照要读的列"（xid、subxid 计数、statusFlags）从行式的 PGPROC/PGXACT 里抽出来，变成三个按 `pgxactoff` **紧凑连续**索引的数组，且只有"在 ProcArray 里的进程"占据条目（进程退出即压缩）：

```
PG 13 及以前（行式, 按 proc 槽位稀疏分布）:
  PGXACT[0]{xid,xmin,flags,...} ... PGXACT[57]{...} ...... PGXACT[4096]{...}
  拍快照: 跳读 4096 个槽位, 每个一次缓存行加载, 绝大多数是空的

PG 14 起（列式, 稠密）:
  ProcGlobal->xids[]:        [xid0][xid1][xid2]...[xidN]   ← 只含在场进程
  ProcGlobal->subxidStates[]:[s0][s1][s2]...[sN]
  ProcGlobal->statusFlags[]: [f0][f1][f2]...[fN]
  拍快照: 顺序扫 N 个活跃条目, 一个缓存行装 16 个 XID
```

收益：主循环变成顺序内存扫描，硬件预取器全速工作，几百连接的活跃集合只需几十次缓存行加载。配合快照复用，官方基准里数千连接的只读吞吐提升数倍，是近十年最重要的可扩展性改进之一。
...
### 8.3.7 快照的生命周期管理：注册、活动栈与导出

8.4 走读的函数开头有一行 `Assert(snapshot->regd_count > 0 || snapshot->active_count > 0)`——它牵出快照的另一半故事：**谁在保证"我正在用的快照不会失效"？**`snapmgr.c` 用两套引用计数管理：

- **ActiveSnapshot 栈**（`PushActiveSnapshot()`/`PopActiveSnapshot()`，`snapmgr.c:682/775`）：执行器跑一条语句时压栈的"当前快照"，`GetActiveSnapshot()`（`snapmgr.c:800`）随取随用。栈结构对应嵌套执行：外层查询调用一个函数、函数内又执行 SQL（可能带自己的快照），退出时逐层弹回；
- **注册快照**（`RegisterSnapshot()`/`UnregisterSnapshot()`，`snapmgr.c:824/866`）：把快照挂到 ResourceOwner 上的长期引用——游标（portal）是典型客户：`DECLARE CURSOR` 后事务里后续语句各有新快照，但游标 FETCH 必须继续用声明时的旧快照，靠注册计数保命。

两个计数共同决定 `MyProc->xmin` 何时可以推进：`SnapshotResetXmin()` 在事务内所有快照都注销后清掉公示的 xmin（READ COMMITTED 长事务因此不会一直钉住视界——除非有游标常年不关，那是 8.3.8 事故的常见变体）。

**快照导出/导入**（`ExportSnapshot()`，`snapmgr.c:1115`；SQL 层 `pg_export_snapshot()` 与 `SET TRANSACTION SNAPSHOT`）：把快照序列化到 `pg_snapshots/` 文件，另一个会话导入后与导出者**看到完全相同的世界**。两大用户：`pg_dump --jobs`（多进程并行导出必须共享一致视图）与逻辑复制的初始表同步。导入方通过 `ProcArrayInstallImportedXmin()`（`procarray.c:2479`）原子地检查"导出者还活着"并接过其 xmin——防止导出者退出后视界前移、快照变成悬空引用。

### 8.3.8 what-if：一个长事务钉住 xmin 之后
...
5. **子事务慢路径放大**（若 A 同时滥用 SAVEPOINT）：见 8.4.6；
...
`HeapTupleSatisfiesMVCC` 只是 `heapam_visibility.c` 里可见性函数族的一员，统一入口是 `HeapTupleSatisfiesVisibility()`（`heapam_visibility.c:1732`）按 `snapshot->snapshot_type` 分发。同族成员共用"hint → 自己 → 状态 → CLOG"的骨架，差异全在"用什么当'现在'"：

| 函数 | 位置 | "现在"的定义 | 典型用户 |
|---|---|---|---|
| SatisfiesMVCC | `:939` | 快照（xip 黑名单）+ curcid | 普通查询 |
| SatisfiesSelf | `:297` | 瞬时：自己的已做修改全可见（不比 curcid） | 唯一约束的自查、RI 触发器 |
| SatisfiesUpdate | `:511` | 瞬时 + 返回**多值**（TM_Ok/Invisible/BeingModified/Updated/Deleted...） | UPDATE/DELETE/行锁定位目标 |
| SatisfiesDirty | `:759` | 瞬时且**连未提交的也看**，通过 snapshot 回传 xmin/xmax 告知"谁在写" | 唯一索引冲突检查、EPQ 定位最新版本 |
| SatisfiesToast | `:452` | 几乎总可见（主表行可见性已把关） | TOAST 表内部读取 |
| SatisfiesVacuum | `:1113` | OldestXmin 视界，返回 DEAD/RECENTLY_DEAD/LIVE/INSERT(DELETE)_IN_PROGRESS | VACUUM/剪枝/ANALYZE |

三个成员值得多说一句：

- **SatisfiesUpdate** 是写路径的裁判：`heap_update` 先用它给目标行定性——TM_BeingModified 触发行锁等待（8.7.4），TM_Updated 触发 RC 的 EPQ 或 RR 的 40001（8.3.6 的分叉点即在其返回值上）。它必须"瞬时"而非快照语义：写操作面对的是**当前**世界，改一个快照里的旧版本毫无意义；
- **SatisfiesDirty** 是"脏读"在 PG 里唯一的合法出口：INSERT 撞唯一索引时必须看见并发未提交的同键行（否则两个未提交插入互相看不见、双双成功、约束破产）；看见后把对方 XID 通过快照结构回传，调用者据此去等对方的事务锁——"等它见分晓再裁决"；
- **SatisfiesVacuum** 把"对谁可见"换成"还有没有人可能看见"：xmax 已提交但仍落在某快照窗口内 → RECENTLY_DEAD（不可清）；这正是 8.3.8 长事务钉 xmin 危害的判定现场。

InnoDB 对照：这七种语义 InnoDB 同样全有，但形态是**读模式**（一致性读 vs 当前读 vs 半一致读）叠加在同一套 undo 回溯逻辑上；PG 把它们摊成七个显式函数，代价是骨架代码重复七遍（源文件里同构的 if 树反复出现），收益是每种语义可独立演化、独立加注释——`heapam_visibility.c` 头部 90 行的总注释是理解全族的最佳入口。

### 8.4.6 what-if：第 65 个子事务——suboverflowed 性能悬崖
...
5. 窗口宽度放大器：如果同时有个长事务把 xmin 摁得很低（8.3.8），[xmin, xmax) 窗口极宽，落进窗口的 XID 更多，慢路径命中率进一步升高——两个反模式相乘而非相加；
...
- 可见性是一族七个函数（MVCC/Self/Update/Dirty/Toast/Vacuum/...），骨架相同、"现在"的定义不同：写路径用瞬时语义（SatisfiesUpdate），唯一约束要看未提交（SatisfiesDirty），VACUUM 问的是"还有没有人可能看见"（SatisfiesVacuum）。
...
本章出场的 SLRU 成员及其在正文中的角色，集中列一遍（目录名即 `$PGDATA` 下的子目录）：

| SLRU | 目录 | 记什么 | 崩溃安全 | 本章出场处 |
|---|---|---|---|---|
| clog | pg_xact | 每 XID 2 bit 命运 | 是（提交前 WAL 先行） | 8.4.4 查询、8.5.1 |
| subtrans | pg_subtrans | 子→父 XID 映射 | 否（崩溃即弃） | 8.1.3 写入、8.4.3 慢路径 |
| multixact offsets/members | pg_multixact | 组合行锁成员 | 是（9.3 起） | 8.7.3 |
| commit_ts | pg_commit_ts | 提交时间戳（track_commit_timestamp） | 是 | （逻辑复制冲突裁决用） |
| serial | pg_serial | SSI 老已提交事务摘要 | 否 | 8.9.4 ConflictOut 的 SLRU 分支 |

观测入口是 PG 13+ 的 `pg_stat_slru` 视图：`blks_hit/blks_read` 之比是缓冲够不够的直接证据——8.4.6 的 suboverflow 事故在这里表现为 subtrans 行的 `blks_read` 陡增；multixact 两行的读量陡增则多半是"共享行锁+外键"热点（8.7.3）。PG 17 起各成员缓冲量可用对应 GUC 单独调，调大 subtransaction_buffers 是 SAVEPOINT 重载系统的常见止血（治本仍是改应用）。
...
- 自旋失败会指数退避乃至 sleep（`s_lock.c` 的 `perform_spin_delay`，`s_lock.c:131` 起）：每 `spins_per_delay` 次（初值 100，按争用历史在 10~1000 间自适应）失败就 sleep 一次，睡眠时长从 1ms 起随机指数增长、封顶 1s（`s_lock.c:60-61`）；累计睡眠过久报 PANIC "stuck spinlock"。自适应逻辑（`s_lock.c` 尾部 `update_spins_per_delay`）：单核机器上自旋纯属浪费（持有者根本没在跑），多核上多转几圈常能免掉睡眠——运行时按"最终靠自旋拿到 vs 靠睡眠拿到"的比例调整。因此**持锁区内绝不允许任何可能阻塞/出错的操作**，连 elog 都不行。
...
**变体一族**（列出以备走读他章时对号入座，本章不展开）：`LWLockConditionalAcquire()`（`lwlock.c:1321`）——拿不到立刻返回 false，绝不排队（buffer 的条件 pin、WAL 的机会主义写盘用它）；`LWLockAcquireOrWait()`（`lwlock.c:1378`）——拿不到就等到锁被释放，但**醒来后不再尝试获取**直接返回（专为 `XLogFlush` 定制：等别人刷完 WAL 后重新检查刷盘进度，往往发现活已被干完）；`LWLockWaitForVar()`/`LWLockUpdateVar()`（`lwlock.c:1566/1702`）——把一个 64 位变量的变更通知嫁接在锁的等待队列上（WAL 插入位置的推进通知）。三个变体全部复用同一个状态字与队列协议，是"一把锁的原语可以长出多少接口"的样本。
...
### 8.6.4 冲突之后：WaitOnLock → ProcSleep 等待路径走读

`LockAcquireExtended` 第 6 步冲突后调 `JoinWaitQueue()`（`proc.c:1209`，持分区锁内完成入队/或发现其实能立即拿到），随后 `WaitOnLock()`（`lock.c:1940`）设置好出错清理（保证被 cancel 时能把自己摘出等待队列）再进入真正的睡眠循环 `ProcSleep()`（`proc.c:1378`）。这段路径是 8.6（锁）与 8.8（死锁）的接缝，值得单独走读：
...
ProcSleep(LOCALLOCK *locallock)
{
    ...
    if (!InHotStandby)
    {
        ...
        {   /* 1425-1435: 一次系统调用同时武装两个定时器 */
            ...
            enable_timeout_after(DEADLOCK_TIMEOUT, DeadlockTimeout);   // 1438

        /* 1454: 复用定时器取到的时间戳当 waitStart(pg_locks 的等待起点) */
    ...
    do
    {
        (void) WaitLatch(MyLatch, WL_LATCH_SET | WL_EXIT_ON_PM_DEATH, 0,
                         PG_WAIT_LOCK | locallock->tag.lock.locktag_type);  // 1524
        ResetLatch(MyLatch);
        /* check for deadlocks first, as that's probably log-worthy */
        if (got_deadlock_timeout)
        {
            deadlock_state = CheckDeadLock();          // 1530
            got_deadlock_timeout = false;
        }
        CHECK_FOR_INTERRUPTS();
        ...
        myWaitStatus = *((volatile ProcWaitStatus *) &MyProc->waitStatus);  // 1541

        if (deadlock_state == DS_BLOCKED_BY_AUTOVACUUM && allow_autovacuum_cancel)
        {   /* 1547-1567: 取出 autovacuum 信息后立刻放锁, 再发 SIGINT 取消之 */
            ...
        }
        ...
...
- **睡在 latch 上而非信号量上**（对比 LWLock 睡信号量）：latch 可被多种事件置位（锁授予、超时信号、cancel），且 `WaitLatch` 返回后统一走 `CHECK_FOR_INTERRUPTS()`——重量级锁等待必须是**可取消**的（用户 Ctrl-C、statement_timeout），而 LWLock 临界区恰恰相反（HOLD_INTERRUPTS）。两层锁对中断的相反态度，源自持有时长的量级差；
- **唤醒即持有**（`proc.c:1472-1478` 注释）：与 LWLock"被唤醒者重抢"相反，重量级锁由释放者在 `ProcLockWakeup` 里**直接把锁授予**等待者（改 waitStatus 为 OK）再置 latch——因为这里的公平性优先级高于吞吐（8.6.2 ⑥的红线再次出现），且授予动作本来就要在分区锁临界区里改共享状态，顺手做掉没有额外成本；
- `waitStatus` 用 volatile 读且每轮只读一次（`proc.c:1536-1541`）——它会被授予者/死锁检测异步改写；
- **DS_BLOCKED_BY_AUTOVACUUM 的特殊通道**：死锁检测顺带发现"挡我的是一个 autovacuum"时，这里直接给它发 SIGINT（非回卷保护的 autovacuum 会礼让位置）——用户事务优先于后台维护的具体实现。

**`CheckDeadLock()`（`proc.c:1887`）**——定时器到点后的现场：

```c
static DeadLockState
CheckDeadLock(void)
{
    /* 1902: 按分区号升序拿下全部 16 个锁表分区 —— 遵守 LWLock 固定顺序纪律 */
    for (i = 0; i < NUM_LOCK_PARTITIONS; i++)
        LWLockAcquire(LockHashPartitionLockByIndex(i), LW_EXCLUSIVE);

    /* 1915: 先看自己是否已在等待期间被授予锁 —— 醒得巧, 白跑一趟 */
    ...
    result = DeadLockCheck(MyProc);                    // 1927: 8.8.2 的主线

    if (result == DS_HARD_DEADLOCK)
    {
        /* 1944: 把自己摘出等待队列; waitStatus 被置 ERROR,
         * ProcSleep 返回后由上层报 "deadlock detected" */
        RemoveFromWaitQueue(MyProc, LockTagHashCode(&(MyProc->waitLock->tag)));
    }

check_done:
    /* 1966: 逆序放锁: 别人按升序拿, 逆序放可让最后一把锁的等待者凑齐所有锁,
     * 也避免 LWLockRelease 内部数组搜索的 O(N^2) */
...
`proc.c:1893-1900` 的注释交代了两条隐含纪律：死锁检测中断绝不能在自己持有分区锁的代码里被启用（否则自己等自己，永久挂死）；LWLockAcquire 自带的 HOLD_INTERRUPTS 保证检测过程不被 cancel 打断。1902 行的升序获取正是 8.10 不变式 I-7"固定顺序"的最大规模实践——16 把锁一次拿齐。
...
- 等待路径：JoinWaitQueue 入队 → ProcSleep 睡 latch（可取消、武装 deadlock/lock 双定时器）；重量级锁由释放者直接授予（公平优先），与 LWLock 的"唤醒重抢"（吞吐优先）形成对照；CheckDeadLock 升序锁全部 16 分区、逆序释放。
...
| 已持有 \ 请求 | KEY SHARE | SHARE | NO KEY UPDATE | UPDATE |
|---|---|---|---|---|
| KEY SHARE | | | | ✗ |
| SHARE | | | ✗ | ✗ |
| NO KEY UPDATE | | ✗ | ✗ | ✗ |
...
第一行是整套设计的目的所在：**KEY SHARE（外键检查）只与 UPDATE（删行/改键）冲突**——改非键列的普通 UPDATE（NO KEY UPDATE）畅通无阻。矩阵不以数据表形式存在于源码，而是编码在 `tupleLockExtraInfo[]`（`heapam.c:134`）里：

```c
static const struct { LOCKMODE hwlock; int lockstatus; int updstatus; }
tupleLockExtraInfo[] = {
    [LockTupleKeyShare]       = { AccessShareLock,     MultiXactStatusForKeyShare,    -1 },
    [LockTupleShare]          = { RowShareLock,        MultiXactStatusForShare,       -1 },
    [LockTupleNoKeyExclusive] = { ExclusiveLock,       MultiXactStatusForNoKeyUpdate,
                                                       MultiXactStatusNoKeyUpdate },
    [LockTupleExclusive]      = { AccessExclusiveLock, MultiXactStatusForUpdate,
                                                       MultiXactStatusUpdate },
};
```

三列各有讲究：`hwlock` 把四种行锁模式**映射到四个表锁模式**——等待冲突判定直接复用 8.6.3 的 LockConflicts 矩阵（行锁不进锁表，但"冲突与否"的判定逻辑白嫖了锁表的矩阵，妙）；`lockstatus`/`updstatus` 是该模式作为 multixact 成员时的状态编码（区分"只是锁"和"锁且更新"，`updstatus = -1` 表示该模式不允许更新——KeyShare/Share 是纯读保护）。
...
省略说明：`heap_lock_tuple` 全文近 700 行，其中约六成处理 multixact 模式合并与升级等待的组合爆炸（4 种已有状态 × 4 种请求模式 × wait_policy），主干即上述三步，故不逐行走读；`heap_update` 内嵌的"更新即加锁"路径与此同构。这片组合爆炸区的凶险有案可查：**锁升级**（已持 KEY SHARE 再请求 UPDATE 级）曾造成"假死锁"——升级者先释放排队票再重新排队，与第三方交错出本可避免的环，2019 年才由 commit `de87a084c0`/`8b21b416ed`（"Avoid spurious deadlocks when upgrading a tuple lock"）修正：已持有较弱行锁者升级时跳过排队票直接进入等待，消除窗口。

### 8.7.5 what-if：如果没有 tuple 锁这张"排队票"？

假想把 8.7.4 第 2 步的 LOCKTAG_TUPLE 锁拿掉，只保留"等持有者事务锁"：

1. 行 R 被 T0 锁住，T1、T2、T3 相继来抢：三者都对 XID_T0 的事务锁挂起共享申请；
2. T0 提交，三者**同时**醒来，同时重读 R、同时发现"无人持有"、同时尝试写 xmax——只有一个成功（页面排他锁裁决），另两个发现 xmax 又变了，回到第 1 步改等新持有者；
3. 问题一（惊群+活锁倾向）：每轮唤醒 N 个等待者、成功 1 个，浪费 N-1 次页面访问；问题二（**饥饿**）：胜者由调度时机决定，倒霉的 T3 可能在每一轮都输——行锁等待是分钟级的（8.6.2 ⑥的红线），无界饥饿不可接受；
4. 排队票的修复：等待前先拿 R 的 tuple 锁（重量级锁，FIFO 队列）。T1 得票，T2、T3 卡在 tuple 锁队列里**根本不去等事务锁**；T0 提交后只有 T1 醒来接手行、随即释放票，T2 递补。每轮恰好一人上场，顺序即到达顺序；
5. 代价极小：tuple 锁只在"有人排队"的瞬间存在（拿到行锁即释放），锁表里最多 (热点行数 × 1) 条，不随等待者数增长——这就是 pg_locks 里 tuple 锁总是一闪而过的原因。

InnoDB 的对应机制是锁队列本身（记录锁结构里的 waiting 队列天然 FIFO），因为它的行锁本来就在锁表里——PG 把行锁挪出锁表后，公平性缺口就得靠这张临时票补回来。**把状态挪出共享内存的每个设计，都要为丢失的"全局视野"打一个补丁**：行锁的补丁是 tuple 排队票，fastpath 的补丁是强锁计数器+搬运（8.6.3），快照的补丁是 xactCompletionCount（8.3.4）——同一主题的三次变奏。
...
把这笔账浓缩成一张对比表（设 B=每秒锁等待次数，D=检测一次的成本≈冻结 16 分区+DFS，p=等待超过 1s 的比例）：

| 策略 | 每秒检测成本 | 死锁发现延迟 | 无冲突负载的干扰 |
|---|---|---|---|
| 即时检测（InnoDB 默认） | B × D | ≈0 | 每次等待都冻结锁体系 |
| 懒触发（PG） | B × p × D（p 通常 <0.001） | ≤ deadlock_timeout | 零（不等满 1s 不付费） |
| 纯超时（innodb_deadlock_detect=OFF） | 0 | lock_wait_timeout（默认 50s） | 零，但误杀慢等待 |
...
### 8.9.1 异常谱系：同一组转账例子过四个隔离级别

先建立实验台。账户表 `acct(id, bal)`，初始 A=100、B=100，业务约束：**允许单户透支，但 A+B ≥ 0 必须恒成立**。两类经典异常各推演一遍：

**异常一：lost update（丢失更新）。**两个会话都执行"读余额、算新值、写回"：

```sql
-- T1:                                   -- T2:
SELECT bal FROM acct WHERE id='A';       SELECT bal FROM acct WHERE id='A';  -- 都读到 100
UPDATE acct SET bal = 60 WHERE id='A';   UPDATE acct SET bal = 70 WHERE id='A';
COMMIT;                                  COMMIT;
```

| 隔离级别 | 行为 | 代码分叉点 |
|---|---|---|
| PG READ UNCOMMITTED | = READ COMMITTED（PG 不实现脏读） | `GetTransactionSnapshot` 不区分两者 |
| PG READ COMMITTED | T2 的 UPDATE 阻塞在行锁上（等 XID_T1 事务锁）；T1 提交后，T2 走 **EPQ**：拿行的最新版本（bal=60）重验 WHERE 后**覆盖写入 70**——T1 的扣款丢了 | `heap_update` 返回 TM_Updated → `ExecUpdate` 因 `!IsolationUsesXactSnapshot()` 走 `EvalPlanQual`（`execMain.c:2687` 起） |
| PG REPEATABLE READ | T2 的 UPDATE 等锁后**报错 40001** "could not serialize access due to concurrent update"——应用重试即无丢失 | 同一分叉的另一支：`IsolationUsesXactSnapshot()` 为真 → ereport |
| PG SERIALIZABLE | 同 RR（先于 SSI 检测就被"并发更新"规则拦下） | 同上 |
| InnoDB RR 对照 | UPDATE 用**当前读**：等锁后静默基于最新版本重执行——单条 UPDATE 语义同 PG RC；但 SELECT 不加锁时，"读-算-写"模式照样丢更新 | InnoDB 的写路径永远当前读，无 EPQ 概念 |

注意：若把 UPDATE 写成 `SET bal = bal - 40` 的**原子读改写**，PG RC 的 EPQ 会基于新版本重算，不丢更新——lost update 只在"应用层先读后写"的形态下出现。这是"RC 够不够用"争论的核心细节。

**异常二：write skew（写偏斜）。**两个会话各自检查约束后扣**不同**的账户：

```sql
-- T1: A 扣 150 (先验 A+B-150>=0)         -- T2: B 扣 150 (先验 A+B-150>=0)
BEGIN ISOLATION LEVEL REPEATABLE READ;    BEGIN ISOLATION LEVEL REPEATABLE READ;
SELECT sum(bal) FROM acct;   -- 200 ✓     SELECT sum(bal) FROM acct;   -- 200 ✓
UPDATE acct SET bal = bal-150             UPDATE acct SET bal = bal-150
  WHERE id = 'A';                           WHERE id = 'B';
COMMIT;  -- 成功                           COMMIT;  -- 也成功 → A+B = -100, 约束破产!
```

| 隔离级别 | 行为 |
|---|---|
| PG RC / RR | **双双成功**。两人改的是不同的行：行锁不碰、EPQ 不触发、"并发更新"检查不触发——快照隔离的所有防线都只盯"同一行"，而 write skew 的冲突发生在**读集与写集的交叉**上 |
| PG SERIALIZABLE (SSI) | T1 读全表时对扫过的对象留 SIREAD 锁；T2 的 UPDATE 命中 T1 的 SIREAD → 记边 T1 --rw--> T2；对称地 T2 --rw--> T1。环闭合，后动作者在 `OnConflict_CheckForSerializationFailure` 或提交时报 **40001**（reason: "as a pivot"） |
| InnoDB RR | 同 PG RR，双双成功（write skew 照样发生——InnoDB 的 RR 也是 SI 底子） |
| InnoDB SERIALIZABLE | 所有 SELECT 隐式变成 `LOCK IN SHARE MODE`（2PL）：T1 的读给两行都加了共享 next-key 锁；T2 的 UPDATE 要 X 锁 → 阻塞；T2 的读也锁了两行，T1 的 UPDATE 也阻塞——**经典 2PL 死锁**，检测后杀一个。约束保住了，代价是读写互相阻塞 + 死锁回滚 |

把两张表纵向对齐就得到本章的理论总纲：**SI（PG RR）挡得住"同行冲突"类异常（脏写、不可重复读、经典幻读、单行 lost update），挡不住"读写交叉"类异常（write skew 及只读事务异常）；2PL 全挡但读写互阻；SSI = SI 的并发 + 事后精确检测读写交叉**。经典的值班医生例子（两名医生同时请假，各自快照里都还有别人值班）与转账 write skew 同构，不再重复。

### 8.9.2 危险结构：两条连续的 rw 边

理论核心（Fekete et al. 2005 + Cahill et al., SIGMOD 2008——`README-SSI` 列为第一参考文献，PG 9.1 是该论文的首个工业级实现，commit `dafaa3efb7`，2011-02-07，Kevin Grittner & Dan Ports）：快照隔离下每个串行化异常的依赖环里，**必然存在一个"枢轴"事务 Tpivot，身上挂着两条连续的 rw 反依赖边**，且 Tout 必须是环中最先提交的：
...
rw 反依赖（rw-antidependency）：T1 读了某数据的旧版本、T2 并发写出了新版本 ⇒ T1 --rw--> T2（"T1 必须排在 T2 前面"的串行化约束）。SSI 只需：追踪并发事务间的 rw 边；一旦某事务集齐"一进一出"两条 rw 边（成为 pivot）且提交顺序条件满足，就地回滚其中一个。**不追踪 wr/ww 依赖、允许假阳性**（危险结构不一定真闭合成环）——用少量误杀换取极低的追踪成本，这是 SSI 实用化的关键取舍。

**只读事务异常**：定理里 Tin 可以是纯只读事务——这是反直觉的重点，用 Fekete/O'Neil 2004 的经典例子验证。支票户 x=0、储蓄户 y=0，透支收 1 元罚金：

```
T1(存款):   R(y)=0;  W(y=20);  COMMIT           -- 给储蓄户存 20
T2(取款):   R(x)=0;  R(y)=0;                    -- 总余额 0, 取 10 要罚 1
            W(x=-11); COMMIT                    -- (T2 拍照早于 T1 提交)
T3(对账,只读): R(x)=0; R(y)=20; COMMIT          -- 拍照于 T1 提交后、T2 提交前
```

没有 T3 时这个执行是可串行化的（等价于串行 T2→T1：先取款被罚、后存款）。T3 一来全乱：T3 看到了 T1 的存款（T3 排 T1 后）却没看到 T2 的取款（T3 排 T2 前）⇒ 串行序须为 T1→T3→T2；但 T2 读到 y=0 意味着 T2 排在 T1 的存款前 ⇒ T2→T1。矛盾——**一个什么都没写的事务，让本来可串行化的历史变得不可串行化**（它观察到了"罚金已扣而存款未至"这个任何串行执行都不会产生的世界）。依赖图：T3 --rw--> T2（T3 读了 x 的旧版）、T2 --rw--> T1（T2 读了 y 的旧版），T2 是 pivot，Tout=T1 最先提交——恰好命中危险结构。这解释了两件事：SSI 为什么必须给只读事务也记 SIREAD 锁（Tin 侧的边要采集）；以及 8.9.6 里"Tin 只读且快照晚于 Tout 提交则无害"豁免的精确边界——本例中 T3 的快照晚于 T1 提交，豁免不成立，必须报 40001。

### 8.9.3 SIREAD 锁：不阻塞任何人的谓词锁

`src/backend/storage/lmgr/predicate.c`（约 5000 行，配 `README-SSI`）。SERIALIZABLE 事务每读一个元组/页/整表，就调 `PredicateLockTID()`（`predicate.c:2550`）/ `PredicateLockPage()`（`predicate.c:2528`）/ `PredicateLockRelation()`（`predicate.c:2505`）在独立的谓词锁表（不在 lock.c 的锁表里）登记 SIREAD 锁。理解它的三个关键性质：

1. **纯登记，零阻塞**：SIREAD 锁与任何锁都不"冲突"，它只是给未来的写者留一张"我读过这里"的字条。与 InnoDB SERIALIZABLE 的共享 next-key 锁（真的挡写）是检测/预防两种哲学的分野；
2. **三级粒度 + 自动升级**：元组锁多了并成页锁、页锁多了并成表锁，阈值由 `max_pred_locks_per_page`/`per_relation`（`guc_parameters.dat:2089/2098`，SIGHUP 级可调）控制，总量受 `max_pred_locks_per_transaction`（决定共享内存池大小）约束。升级牺牲精度换内存：页锁会把"同页其他行的写"误判为冲突——假阳性率随升级上升，这是 SERIALIZABLE 下 40001 重试率与内存参数直接挂钩的原因；
3. **锁的位置编码了"谓词"**：索引范围扫描在**索引页**上留 SIREAD 锁——锁住"我搜过的范围"，未来插入该范围的行必先改这个索引页，命中字条：这就是 PG 版的谓词锁（功能上对应间隙锁的检测面）。B 树页分裂时锁要跟着搬（`PredicateLockPageSplit()`，`predicate.c:3073`——分裂把"范围"劈成两页，两页都得继承字条）。全表扫描则直接留表级锁——所以 SERIALIZABLE 下顺序扫描大表极易与任何写冲突，实践中要保证查询走索引，这是 SSI 调优的第一条军规。

SIREAD 锁还必须**在事务提交后继续保留**（直到所有并发事务结束），因为 rw 冲突可能在读者提交后才被写者触发（8.10 不变式 I-9）；旧已提交事务的摘要最终挤进 SLRU（pg_serial，8.5.3 的表）。

### 8.9.4 CheckForSerializableConflictOut 主线走读

读侧采集点：可见性判断附近，当我读到的元组**被某个并发事务创建（对我不可见）或删改（新版本对我不可见）**时，heap AM 把那个事务的顶层 XID 传进来（`predicate.c:3939-3949` 函数头注释）。`predicate.c:3952`：
...
CheckForSerializableConflictOut(Relation relation, TransactionId xid, Snapshot snapshot)
{
    if (!SerializationNeededForRead(relation, snapshot))
        return;                                         // 3958: 非 SSI 事务零开销

    if (SxactIsDoomed(MySerializableXact))              // 3962: 已被判死缓期执行
        ereport(ERROR, ...40001 "Canceled on identification as a pivot"...);
    ...
    if (!sxid)                                          // 3982: 不在活跃 SSI 表里
    {
        /* 3990: 查 pg_serial(SLRU) 里的"老已提交事务"摘要 */
        ...
        if (conflictCommitSeqNo != 0)
        {
            if (conflictCommitSeqNo != InvalidSerCommitSeqNo && ...)
                ereport(ERROR, ..."Canceled on conflict out to old pivot"...);
            ...
            MySerializableXact->flags |= SXACT_FLAG_SUMMARY_CONFLICT_OUT;
        }
        LWLockRelease(SerializableXactHashLock);
        return;                                         // 对方不是 SSI 事务, 无事
    }
    sxact = sxid->myXact;
    if (sxact == MySerializableXact || SxactIsDoomed(sxact))
        { ...return; }                                  // 4020: 自己/将死者不算

    if (SxactHasSummaryConflictOut(sxact))              // 4033: 对方已有出边且被摘要
        { ... 判死或报错 ... }

    /* 4057: 只读优化: 我只读 && 对方已提交 && 其出边冲突者不早于我的快照 → 无害 */
    if (SxactIsReadOnly(MySerializableXact)
        && SxactIsCommitted(sxact) && !SxactHasSummaryConflictOut(sxact)
        && (!SxactHasConflictOut(sxact)
            || MySerializableXact->SeqNo.lastCommitBeforeSnapshot
               < sxact->SeqNo.earliestOutConflictCommit))
        { ...return; }

    if (!XidIsConcurrent(xid))                          // 4068: 其写已在我快照内
        { ...return; }                                  /* 那是 wr 依赖, 不是 rw */

    if (RWConflictExists(MySerializableXact, sxact))    // 4075: 去重
        { ...return; }

    FlagRWConflict(MySerializableXact, sxact);          // 4086: 建边 me --rw--> sxact
    LWLockRelease(SerializableXactHashLock);
}
```

要点：`XidIsConcurrent`（4068）把"我本来就看得见其写"的情形排除——那是正常的 wr 依赖（我排它后面），只有**并发**的写形成 rw 反依赖；`FlagRWConflict` 内部（`predicate.c:4425-4444`）视对方是否已被摘要化置标志或建边，并立即调 `OnConflict_CheckForSerializationFailure` 检查危险结构。

### 8.9.5 CheckForSerializableConflictIn 主线走读

写侧采集点：任何 INSERT/UPDATE/DELETE 前调用，`predicate.c:4265`：
...
CheckForSerializableConflictIn(Relation relation, const ItemPointerData *tid, BlockNumber blkno)
{
    ...
    if (SxactIsDoomed(MySerializableXact))
        ereport(ERROR, ...40001...);                    // 4273

    MyXactDidWrite = true;                              // 4284

    /* 4286-4293 注释: 必须从细到粗检查, 与粒度升级"先粗后细"的顺序相反,
       两个方向相向而行保证不漏 */
    if (tid != NULL)
    {
        SET_PREDICATELOCKTARGETTAG_TUPLE(targettag, ...);
        CheckTargetForConflictsIn(&targettag);          // 4302: 元组级
    }
    if (blkno != InvalidBlockNumber)
    {
        SET_PREDICATELOCKTARGETTAG_PAGE(targettag, ...);
        CheckTargetForConflictsIn(&targettag);          // 4311: 页级
    }
    SET_PREDICATELOCKTARGETTAG_RELATION(targettag, ...);
    CheckTargetForConflictsIn(&targettag);              // 4317: 表级
}
```

`CheckTargetForConflictsIn()`（`predicate.c:4095`）对目标上的每个 SIREAD 锁持有者建边 holder --rw--> me（同样经 FlagRWConflict → OnConflict 检查）。三级都查是因为读者可能在任何粒度留锁（含升级后只剩粗粒度锁的情形）；从细到粗的顺序与升级方向相反（`predicate.c:4286-4293` 注释），保证升级并发进行时至少在一个粒度上相遇。

### 8.9.6 危险结构判定：OnConflict_CheckForSerializationFailure 走读

每次建边时调用（`predicate.c:4465`），检查新边是否让 reader 或 writer 成为致命 pivot。函数头注释（`predicate.c:4446-4462`）给出总纲：危险结构 `Tin --rw--> Tpivot --rw--> Tout` 且 **Tout 最先提交**；若 Tin 只读，还须 Tout 在 Tin 拍快照前提交才有害。正文三段：

```c
    /* ① 4485: writer 已提交且已有出边 → R→W→T2 全链已成、W 不可再杀 */
    if (SxactIsCommitted(writer)
        && (SxactHasConflictOut(writer) || SxactHasSummaryConflictOut(writer)))
        failure = true;

    /* ② 4508-4532: writer 因新边成为 pivot? 遍历其 outConflicts 找 T2 */
    dlist_foreach(iter, &writer->outConflicts)
    {
        SERIALIZABLEXACT *t2 = conflict->sxactIn;

        if (SxactIsPrepared(t2)                          /* T2 已提交/已 prepare */
            && (!SxactIsCommitted(reader)
                || t2->prepareSeqNo <= reader->commitSeqNo)   /* 豁免: R 先于 T2 提交 */
            && (!SxactIsCommitted(writer)
                || t2->prepareSeqNo <= writer->commitSeqNo)   /* 豁免: W 先于 T2 提交 */
    ...
    /* ③ 4547-4578: 对称地, reader 因新边成为 pivot? 遍历其 inConflicts 找 T0 */
    ...
    ...
    if (failure)                                        // 4580
    {
        if (MySerializableXact == writer)
            ...
            ereport(ERROR, ...40001 "Canceled on conflict out to pivot"...);  /* 杀自己(reader) */
        writer->flags |= SXACT_FLAG_DOOMED;             // 4611: 否则给 writer 判死缓
    }
}
```

②中的三条豁免是 SSI 从"论文可用"到"生产可用"的关键工程：豁免一/二对应定理的"Tout 必须最先提交"（谁在 T2 前提交了谁就无害）；豁免三是 PG 原创的只读优化（`README-SSI` 有专节证明：只读的 Tin 若在 Tout 提交后才拍快照，可以被串行化排到 Tout 之后，环不成立）。裁决顺序（4580 起）体现"能杀自己不杀别人、绝不杀已 prepare 者"：已 prepare 的两阶段事务承诺过能提交，只能反过来牺牲 reader；杀不到场的 writer 就打 DOOMED 标记，它下次任何操作或提交时自裁（各采集点开头的 SxactIsDoomed 检查就是刑场）。

**提交前最后一道闸：`PreCommit_CheckForSerializationFailure()`（`predicate.c:4632`）**。建边时查不出的唯一漏网情形是：我是 pivot 的 **out 侧**（Tout），而 pivot 和 Tin 都还没提交——那时三条豁免全都不成立与否未定，只能等提交时刻裁决。走读：
...
PreCommit_CheckForSerializationFailure(void)
{
    if (MySerializableXact == InvalidSerializableXact)
        return;                                         // 4636: 非 SSI 事务零开销

    LWLockAcquire(SerializableXactHashLock, LW_EXCLUSIVE);

    if (SxactIsDoomed(MySerializableXact) && ...)       // 4648: 缓期执行的死刑在此落刀
        ereport(ERROR, ...40001 "during commit attempt"...);

    /* 4659: 我即将成为环中最先提交者(Tout). 找 near: near --rw--> me */
    dlist_foreach(near_iter, &MySerializableXact->inConflicts)
    {
        ...
        {
            /* 4669: 再找 far: far --rw--> near, 即 near 是 pivot */
            dlist_foreach(far_iter, &nearConflict->sxactOut->inConflicts)
            {
                RWConflict farConflict = ...;

                if (farConflict->sxactOut == MySerializableXact   /* 两事务互指成环 */
                    || (!SxactIsCommitted(farConflict->sxactOut)
                        && !SxactIsReadOnly(farConflict->sxactOut)
                        && !SxactIsDoomed(farConflict->sxactOut)))
                {
                    if (SxactIsPrepared(nearConflict->sxactOut))
                    {   /* 4685: pivot 已 prepare 动不得 → 我自杀 */
                        ereport(ERROR, ...40001 "conflict in from prepared pivot"...);
                    }
                    nearConflict->sxactOut->flags |= SXACT_FLAG_DOOMED;  // 4694: 杀 pivot
                    break;
                }
            }
        }
    ...
    MySerializableXact->prepareSeqNo = ++(PredXact->LastSxactCommitSeqNo);  // 4701
    MySerializableXact->flags |= SXACT_FLAG_PREPARED;
    LWLockRelease(SerializableXactHashLock);
}
```

两层循环就是在冲突图里找 `far --rw--> near(pivot) --rw--> me(即将提交的 Tout)` 的完整危险结构。裁决策略（`predicate.c:4680-4683` 注释）：**优先杀 pivot 而不是正在提交的我**——我是提交中的写事务，放我过去保证全局有前进性；若反过来杀我，重试后可能在同一结构上再次失败（活锁）。far 侧的过滤条件同样体现豁免：far 已提交（其提交必晚于……不构成 Tout 最先提交？不——far==me 的互指环直接命中）、far 只读、far 已判死的都不算数。函数末尾给自己盖 `prepareSeqNo`+PREPARED 标志——此后我进入"承诺可提交"状态，别人只能绕开我裁决（8.9.6 ②里 `SxactIsPrepared(t2)` 的判据就来自这里）。

`DEFERRABLE` 只读事务则干脆等一个"绝对安全"的快照再执行，全程免疫回滚，适合报表类长查询。

省略说明：谓词锁的登记/升级/清理（约 2000 行）、pg_serial 摘要化细节、2PC 交互不走读——主线逻辑（读写两个采集点 + 建边即检 + 提交终检）已完整覆盖。

### 8.9.7 what-if：备库上为什么不能用 SERIALIZABLE？

热备上执行 `BEGIN ISOLATION LEVEL SERIALIZABLE` 会直接报错 "cannot use serializable mode in a hot standby"（`predicate.c:1624`，入口 `GetSerializableTransactionSnapshot`，`predicate.c:1611`）。推演为什么这不是"没实现"而是"结构性做不到"：

1. SSI 的正确性要求**看见所有并发事务之间的 rw 边**。备库查询的"并发对手"是主库上的写事务，但双方的账本物理隔离：备库读者的 SIREAD 锁登记在**备库本地**共享内存里，主库的写者永远不会来查（`CheckForSerializableConflictIn` 在主库执行，遍历的是主库的谓词锁表）——**边采集不到**；
2. 反向也断：主库写者的 SERIALIZABLEXACT 状态不进 WAL（SSI 全部状态是易失的共享内存），备库的 `CheckForSerializableConflictOut` 查不到主库事务的 sxact——另一半边也采集不到；
3. "只读事务不需要检测"是错觉：**只读事务异常**（Fekete et al. 2004 的经典结果，`README-SSI` 有转述）证明纯只读事务也能观察到非可串行化状态（Tin 只读的危险结构里，Tin 看到"T2 已提交而 pivot 未提交"的中间世界）。所以备库查询并不因只读而天然安全；
4. 把 SIREAD 锁和冲突图放进 WAL 复制？代价是主库每个 SERIALIZABLE 读者的每次读都要产生 WAL 流量，且备库回放要同步维护全图——读扩展（备库分担读）的初衷直接报废；
5. `README-SSI` 记载的未来方向是折中：主库周期性通过 WAL 告知"安全快照点"（类似 DEFERRABLE 的机制），备库的 SERIALIZABLE 查询只在安全点上执行——正确性由"该快照不可能参与任何危险结构"保证，代价是快照延迟。至今未实现，所以备库上限是 REPEATABLE READ：能保证"看到的是某个一致快照"，不能保证"与主库写事务整体可串行化"。

### 小结

- 异常谱系：SI 挡同行冲突（脏写/不可重复读/单行丢更新），挡不住读写交叉（write skew）；2PL 全挡但互阻；SSI = SI 并发 + rw 边监测 + 事后回滚。转账例子在 RC/RR/SSI/InnoDB-2PL 下的分叉点分别是 EPQ、40001 并发更新、SSI 40001、next-key 锁死锁；
- 危险结构定理：异常环必含两条连续 rw 边的 pivot 且 Tout 最先提交——只测这个模式，允许假阳性；
- 实现三件套：SIREAD 谓词锁（元组/页/表三级 + 索引页范围锁，提交后仍保留）、读写两个采集点（ConflictOut/In，从细到粗防升级漏检）、建边即检 + 提交终检（三条豁免规则大幅降误杀；DOOMED 延迟处决；保 prepared、保提交者）；
- 备库不可用 SSI 是结构性的：rw 边横跨主备两个隔离的内存空间，双向都采集不到，且只读事务并不天然安全。

---

## 8.10 不变式清单

每条给出维护者与违反后果，可当 code review 检查表用：

- **I-1 xmin/xmax 生命周期**：元组的 t_xmin 一经写入永不改变（freeze 只改 infomask 或改成 Frozen 常量）；t_xmax 只能从 Invalid → 某 XID/MultiXact，或被作废（hint INVALID）。维护者：heap_insert/update/delete/lock_tuple。违反后果：可见性判断的两阶段推理（8.4.2）全部失效——例如复用 xmax 槽位而不设 LOCK_ONLY 位会让读者把行锁当成删除。
- **I-2 子 XID > 父 XID**：`AssignTransactionId` 先给祖先分配（8.1.3）。依赖方：`GetSnapshotData` 用 xmax 裁剪时跳过子事务收集（`procarray.c:2251-2254`）、`XidInMVCCSnapshot` 的边界裁剪对子事务的正确性（`snapmgr.c:1870-1876`）。违反后果：窗口边界漏判子事务 → 幻读。
- **I-3 ProcArray 成员资格**：任何"已分配且未完成"的 XID，要么 ≥ 快照 xmax，要么在拍照瞬间出现在 ProcArray 的 xids 数组里（`GetNewTransactionId` 持 XidGenLock 写入数组后才发布；`ProcArrayEndTransaction` 持 ProcArrayLock 排他清除并同时推进 latestCompletedXid）。违反后果：8.3.2 的撕裂快照悖论——运行中事务被误判已完成，未提交数据可见。
- **I-4 提交发布顺序**：CLOG/WAL 先说"已提交"（RecordTransactionCommit），然后才从 ProcArray 消失（ProcArrayEndTransaction）。违反后果：新快照判其"已完成"而 CLOG 说"未提交"→ 已提交行被当作 aborted 丢弃。
- **I-5 快照 xmin 单调性与公示**：`MyProc->xmin` 必须在 ProcArrayLock 内、于快照生效前公示，且事务内只降不升（保持为最老在用快照的 xmin）；全局 OldestXmin 因而不会越过任何在用快照。违反后果：VACUUM 清掉在用版本 → 读到被复用的行指针。
- **I-6 hint bit 的 LSN 联锁**：设置 COMMITTED hint 前，commit 记录必须已刷盘或页面 LSN ≥ commit LSN（`SetHintBitsExt`，`heapam_visibility.c:152-166`）。违反后果：异步提交 + 崩溃 = 页上留下假"已提交"章，未提交数据复活。
- **I-7 LWLock 纪律**：多把 LWLock 按全局固定顺序获取；持有期间不做 I/O 等待、不进重量级锁、不 elog(ERROR)（HOLD_INTERRUPTS 保护下 longjmp 会带锁逃逸）。违反后果：无检测的死锁——两个 backend 互等 LWLock 只能靠人肉 gdb，生产上表现为"数据库整体卡死且无任何日志"。
- **I-8 弱锁-强锁发布协议**：弱锁方"读强锁计数 → 写 fastpath 槽"必须整体在 fpInfoLock 内；强锁方"递增计数 → 逐 proc 扫槽"顺序不可颠倒（8.6.3）。违反后果：双方同时走快路，DDL 与 DML 并发下锁矩阵被架空——曾有真实 bug（commit `7db285afc9` "Fix stale-pointer problem in fast-path locking logic" 等一串修复）。
- **I-9 SIREAD 锁跨提交存活**：SERIALIZABLE 事务提交后其 SIREAD 锁保留到所有并发事务结束。违反后果：先提交的读者的 rw 出边采集不到 → write skew 漏检 → SERIALIZABLE 名不副实。
- **I-10 冻结不可见前提**：只有对所有现存快照都可见的元组才能冻结（xmin 早于全局视界）。违反后果：冻结=无条件可见（8.4.2 [X6]），过早冻结让某些快照看见"未来"。
- **I-11 行锁等待的排队票纪律**：等待行锁前必先持有该元组的 LOCKTAG_TUPLE 锁，拿到行锁后立即释放票（8.7.4/8.7.5）。违反后果：热点行上的无界饥饿与惊群；升级场景处理不当则产生假死锁（de87a084c0 修的正是这里）。
- **I-12 multixact 不可变**：已创建的 multixact 成员集永不修改，加成员=造新 multi 换掉 xmax（8.7.3）。违反后果：members SLRU 是定长追加区，原地扩会踩到后续 multi 的成员；并发读者持旧 multi ID 解出错误成员集，可见性判断（[M3]）随之错乱。
...
| 版本 | commit | 内容与教训 |
|---|---|---|
| 9.1 (2011-02-07) | `dafaa3efb7` | **SSI 落地**（Kevin Grittner & Dan Ports）。学术论文（SIGMOD 2008）到工业实现只隔三年；PG 由此成为第一个"可串行化但读写互不阻塞"的主流数据库。工程关键不在定理而在豁免规则与摘要化（8.9.6）——纯定理实现会误杀到不可用。 |
| 9.2 (2011-05-28) | `3cba8999b3` | **fastpath 弱锁**（Robert Haas）。高并发只读压测发现锁表分区锁是第一瓶颈后的对症下药；随后一年内跟着一串修复（`53c5b869b4`、`07ab1383e3`、`7db285afc9`），印证无锁化发布协议的调试难度。PG 18 的 `c4d5cb71d2`（2024-09-21）把固定 16 槽扩展为随 max_locks_per_transaction 伸缩——分区表时代的补课。 |
| 9.3 (2013-01-23) | `0ac5ad5134` | **外键锁并发改造**（Álvaro Herrera）：引入 KEY SHARE/NO KEY UPDATE 两档行锁，multixact 开始容纳 updater 而成为持久关键数据。随后两年爆出 multixact 回卷/freeze 数据损坏 bug 群（如 `b0b263baab`、`f741300c90`、`78db307bb2`），是 PG 史上著名的"功能正确性容易、生命周期正确性难"案例——新的持久数据结构必须完整回答回卷、freeze、崩溃恢复、pg_upgrade 四问。 |
| 9.4 (2013-12/2014-11) | `50e547096c` / `0bd624d63b` | checksum 时代的 hint bit 补丁：`wal_log_hints` GUC 与 XLOG_FPI_FOR_HINT 记录类型——可再生性论证被 checksum 打破后的赎买（8.4.1）。 |
| 12 (2019-03-28) | `2fc7af5e96` | **FullTransactionId 基础设施**（Thomas Munro）：64 位 XID 的"内部先行"路线——控制路径全面 64 位化，磁盘格式不动（8.2.5 的折中落地）。 |
| 14 (2020-08) | `dc7420c2c9`、`941697c3c1`、`623a9ba79b` 等 | **快照可扩展性重构**（Andres Freund）：视界计算移出热路径、稠密数组列式化、xactCompletionCount 快照复用。千连接只读吞吐数倍提升；方法论是"按访问模式而非逻辑归属组织共享内存"（8.3.5）。 |

四段历史合起来还能读出一条元规律：**并发控制子系统的每次大改，都是"先在热路径上省一次共享内存访问，然后花两三年在冷路径的边角（回卷/恢复/升级/2PC）上还债"**。评审新特性时，8.7.3 提到的四问（回卷、freeze、崩溃恢复、pg_upgrade）就是这条规律的检查表。

---

## 8.12 PG vs InnoDB：本章机制对照总表

把散落全章的对照收拢成一页速查（左列为本章小节）：

| 主题 | PostgreSQL | InnoDB | 关键推论 |
|---|---|---|---|
| 多版本存放 (8.0) | 新旧版本平铺堆中，ctid 链前向 | 原地更新 + undo 链后向回溯 | PG 读旧版本零跳转但需 VACUUM；InnoDB 读旧版本要回溯 undo 但清理集中在 purge |
| 事务 ID (8.2) | 32 位 XID + freeze + vxid 惰性分配 | 48 位 trx_id，只读事务免分配 | PG 有回卷停库悬崖；InnoDB 无，但 trx_id 每行占 6 字节（DB_TRX_ID） |
| 回滚成本 (8.1) | O(1)：CLOG 标 2 bit | O(改动量)：undo 反向回放 | PG 大事务回滚瞬间完成；InnoDB 可能比执行还慢 |
| 快照 (8.3) | SnapshotData（xmin/xmax/xip） | ReadView（up/low_limit_id/m_ids） | 结构同构；PG 用于过滤平铺版本，InnoDB 用于选 undo 链深度 |
| 提交状态查询 (8.4/8.5) | CLOG(SLRU) + 元组 hint bit 缓存 | undo 段头 + 锁标志，无全局位图 | PG 首扫盖章产生读放大写；InnoDB 无此现象 |
| 行锁存放 (8.7) | 元组 xmax + infomask，零锁表内存 | lock_sys 内存结构（页级位图压缩） | PG 无锁数量上限/无锁升级，但加锁产生页写+WAL；InnoDB 加锁纯内存 |
| 行锁等待 (8.7) | 等持有者的 TRANSACTION 锁 + tuple 排队票 | 等具体记录锁，队列在锁结构里 | 双方死锁检测都能覆盖行锁等待 |
| 幻读防护 (8.7/8.9) | RR：快照天然防；SER：SSI 谓词锁检测 | RR：next-key 锁（当前读时）；SER：全读加锁 | PG 无间隙锁死锁；InnoDB 无事后回滚 |
| RR 下并发 UPDATE (8.3.6) | 报 40001（先到先得，后来者死） | 当前读静默改最新版本 | 跨库迁移最易踩的语义差 |
| 死锁 (8.8) | 懒触发（睡满 1s 检测）；软边可重排免杀；受害者=先检测者 | 即时检测（可关闭）；受害者=undo 量小者 | 热点冲突下 InnoDB 8.0 建议关检测，殊途同归 |
| 可串行化 (8.9) | SSI：不阻塞、事后 40001、需重试 | 2PL：读加共享 next-key 锁、阻塞+死锁 | PG 高并发下吞吐占优；InnoDB 语义更"传统" |
| 子事务 (8.1.3/8.4.6) | 真 XID + pg_subtrans，溢出拖慢全库读 | undo 回滚点，对他人零影响 | PG 的 SAVEPOINT 有全局隐性成本 |
| 内部锁 (8.6) | spinlock / LWLock（纯原子字实现） | mutex / rw_lock_t（spin+event 混合） | 设计目标一致：短临界区 + 无死锁纪律 |
| 快照热路径 (8.3.4/8.3.5) | 稠密数组 + xactCompletionCount 复用 | trx_sys mutex 下拷贝活跃列表（8.0 起 lock-free 化 m_ids 维护） | 两家都在 2018-2020 年代重做了这条热路径 |
| 长事务病理 (8.3.8) | 主表/索引膨胀（backend_xmin 监控） | undo 膨胀 + history list 变长（HLL 监控） | 病理同构，膨胀部位与监控口径不同 |
| 提交瞬间 (8.1.2) | CLOG 2bit + ProcArray 摘除，O(1) | trx 提交号分配 + undo 标记 purge，O(1) | 双方提交都轻；重活分别推给 VACUUM/purge |

一句话总纲：**PG 把并发控制的成本尽量摊给"空间"（多版本平铺、hint 落页、锁进元组），InnoDB 尽量摊给"时间"（undo 回溯、purge、锁内存维护）**。空间路线的账单由 VACUUM 统一收取——这正是第 10 章的主题。

选型视角的推论（面试常被追问）：写多删多、事务短平快的 OLTP 负载，两家差距不大，InnoDB 的原地更新对二级索引更友好（PG 需 HOT 命中才免索引写）；长事务/大分析查询混跑的 HTAP 场景，PG 的读路径无 undo 回溯之累但膨胀风险更高；需要真可串行化的金融场景，PG 的 SSI 是目前工业界唯一"无阻塞可串行化"选项，InnoDB 的 SERIALIZABLE 在高并发下吞吐坍缩。没有全能冠军，只有账本科目的取舍。

---

## 8.13 常见误区澄清

收拢几条读完本章应当能自行反驳的流行说法：

- **"PG 的 MVCC 是版本链数据库，像 InnoDB 一样回溯"**——错。PG 的 ctid 链是**前向**的（旧指新，供 UPDATE 追最新版），读旧版本不需要任何"回溯"：旧版本就躺在堆里，可见性过滤直接命中（8.0/8.4）。InnoDB 才是后向 undo 回溯；
- **"SELECT 怎么会产生写 I/O？一定是 bug"**——不是。首次扫描盖 hint bit 章（8.4.1）、SSI 的 SIREAD 登记不落盘但 hint 落盘，`EXPLAIN (BUFFERS)` 里纯查询出现 dirtied 页是设计行为；
- **"pg_locks 里没有行锁，说明 PG 行锁是假的"**——行锁在元组头里（8.7.1），pg_locks 只显示等待行锁时的 transactionid/tuple 锁投影。想看真正的行锁得用 pgrowlocks 扩展读页面；
- **"fastpath=true 表示这把锁更弱/不可靠"**——语义完全等同于常规锁，只是存放位置在 PGPROC 槽位；强锁到来时会被搬进锁表继续生效（8.6.3）；
- **"REPEATABLE READ 有幻读，所以要用 SERIALIZABLE 防幻读"**——PG 的 RR（快照隔离）**没有**标准意义的幻读；需要 SERIALIZABLE 防的是 write skew 这类读写交叉异常（8.9.1），两回事；
- **"40001 是数据库出错了"**——serialization failure 是 RR/SSI 的正常工作产物，语义是"请重试"；应用不写重试循环才是 bug（8.9）；
- **"死锁检测会自动预防死锁"**——检测是事后裁决（杀一个），预防要靠应用侧固定加锁顺序；`deadlock_timeout` 调小不减少死锁，只更快发现（8.8）；
- **"SAVEPOINT 是纯客户端概念，随便用"**——第 65 个带写子事务让全库读者进 pg_subtrans 慢路径（8.4.6），PL/pgSQL EXCEPTION 块同罪（8.1.3）。

---

## 自测问题（10 题，带详解）

**1. 为什么 PG 的只读事务不分配真 XID？这个优化在本章出现过几次"兑现"？**

答：`StartTransaction()` 只分配本地 vxid，真 XID 推迟到首次写操作时由 `AssignTransactionId()`（xact.c:637）领取。兑现共四处：①不消耗 32 位 XID 空间、不推进回卷时钟；②不争抢 XidGenLock、不占 CLOG；③别人拍快照时被 `xid == InvalidTransactionId` 一行跳过（procarray.c:2232），不进 xip[]；④只读事务完成不递增 xactCompletionCount（procarray.c:590/768 只在带 XID 完成路径），不作废别人的复用快照（8.3.4）。

**2. 推演：如果 `GetSnapshotData` 不持 ProcArrayLock、边扫边拍，会出什么事？**

答：两类撕裂（8.3.2）。①黑名单漏项：扫描中途提交的事务 T 既被 xmax 放进窗口（若 latestCompletedXid 后读）又不在 xip[]（槽位已清）→ 被判已完成且 CLOG 已 committed → 可见；而与 T 同瞬提交、已被扫进黑名单的 T' 不可见——同一瞬间的两个事务一个可见一个不可见，若二者有依赖则读到"有果无因"的非一致状态。②xmin 偏大：最老活跃事务在被扫到前清了 XID → 我的 MyProc->xmin 虚高 → VACUUM 视界越过我的快照 → 在用版本被清。故快照三要素（latestCompletedXid、各 proc xid、xip[]）必须取自同一互斥瞬间。

**3. hint bit 不写 WAL 为什么不怕崩溃？为什么开 checksum 后又要写 XLOG_FPI_FOR_HINT？**

答：hint bit 是 CLOG 事实的**可再生缓存**：崩溃丢失后任何读者可查 CLOG 重推并重盖；假 COMMITTED 被 LSN 联锁排除（commit 记录未刷盘且页 LSN 无法保序时拒绝设置，heapam_visibility.c:152-166）；假 aborted 不可能（abort 结论只源于真实 CLOG 状态或崩溃推断，且崩溃事务本就按 aborted 处理）。checksum 打破的隐含前提是"torn page 无害"：hint 弄脏的页部分落盘后校验和必然失败、整页被判损坏，故 checkpoint 后首次 hint 脏页须写全页镜像（XLOG_FPI_FOR_HINT，commit 0bd624d63b）——为可检测性付的可再生性税。

**4. `HeapTupleSatisfiesMVCC` 中，为什么"查快照"必须在"查 CLOG"之前？请给一个顺序颠倒后出错的场景。**

答：快照是局部视角（对我算不算完成），CLOG 是全局事实（究竟提交没有），两者可以不一致：事务 T 在我拍照后提交，CLOG=committed 但 T 在我的 xip[] 里。若先查 CLOG：T 的插入被判可见——REPEATABLE READ 事务两次执行同一查询，第二次多出 T 的行，不可重复读/幻读全面破防。正确次序（heapam_visibility.c:1005-1007）先问快照，"在快照里"直接不可见，甚至不必碰 CLOG；hint 已 COMMITTED 时仍要过快照（[X6]/[M5] 分支）同理。

**5. 一个事务开了 100 个带写的 SAVEPOINT，对全库读性能的影响机制是什么？为什么是"悬崖"而不是"斜坡"？**

答：第 65 个子 XID 使该 backend 的 subxids 缓存溢出（PGPROC_MAX_CACHED_SUBXIDS=64，proc.h:43），此后**所有会话**的每张快照都 suboverflowed（GetSnapshotData 采集时任一 overflowed 即整快照标记）。可见性判断中凡落入 [xmin,xmax) 窗口的 XID 都从 SIMD 搜数组退化为 `SubTransGetTopmostTransaction` 逐级读 pg_subtrans SLRU（snapmgr.c:1912），引入 bank 锁争用乃至磁盘 I/O。悬崖成因：64→65 让代价从 O(本地数组) 跳到 O(共享 SLRU 链)，且付费方从肇事者跳到全库读者；若并存长事务把窗口拉宽，二者相乘。

**6. 叙述 LWLock "入队后再试一次"协议，并解释它消除的竞态。**

答：LWLockAcquire（lwlock.c:1207-1247）：第一次 AttemptLock 失败 → LWLockQueueSelf 入队 → **再次** AttemptLock → 成功则 DequeueSelf 拿锁走人，失败才睡信号量。消除的竞态是 lost wakeup：第一次失败到入队之间持有者可能已释放并检查过空队列，无人会再唤醒我。入队先于第二次检查（队列锁的屏障保证顺序对释放者可见），于是要么我第二次抢到（锁已空闲），要么释放者看到我的队列条目负责唤醒——"双方都看不见对方"的交错不存在。配套设计：释放者不移交锁（避免纳秒级临界区被微秒级调度绑架），被唤醒者重新竞争；WAKE_IN_PROGRESS 标志抑制唤醒风暴。

**7. fastpath 锁中，弱锁方和强锁方各自的操作序列是什么？为什么不会双双走快路而漏检冲突？**

答：弱锁方（lock.c:984-1017）：Acquire(自家 fpInfoLock) → 读 FastPathStrongRelationLocks->count[分区] → 为零则写自家槽位 → Release。强锁方（lock.c:1034-1055）：BeginStrongLockAcquire 递增分区计数（封路）→ FastPathTransferRelationLocks（lock.c:2869）遍历全部 PGPROC、逐个 Acquire(其 fpInfoLock) 扫槽搬进共享锁表 → 自己常规冲突检查。穷举交错：弱锁方读到非零 → 退常规路径；读到零 ⇒ 其读先于强锁方递增可见 ⇒ 其槽位写入（同一 fpInfoLock 临界区内）必被强锁方随后对同一把锁的获取所看见并搬走。LWLock 的全屏障语义保证该 Dekker 式协议成立；两边至少一方会退到共享锁表相遇。

**8. 行锁进 xmax 与进锁表两种方案，用一百万行 `SELECT FOR UPDATE` 做量化对比，并列出 xmax 方案的三笔代价。**

答：锁表方案：每行一份 LOCK+PROCLOCK（合计约 2~300 字节）≈ 2~300MB，且 PG 锁表定容（≈ max_locks_per_transaction × max_connections ≈ 默认 6400 条）根本装不下，传统系统靠锁升级自救（并发骤降）。xmax 方案：共享内存 0 增量，锁写进每行本就存在的 xmax+infomask。三笔代价：①写放大——锁一百万行要弄脏几万页并写 XLOG_HEAP_LOCK；②multixact 间接层——共享锁多持有者需组合 ID+双 SLRU+独立回卷 freeze（9.3 后成为持久关键数据，bug 高发区）；③xmax 语义重载——删除者/锁定者/组合三义，每次可见性判断都要先排除 LOCK_ONLY（heapam_visibility.c [M2][M3] 分支），读者替锁买单。等待机制也因此绕道：等行锁=对持有者 XID 拿 LOCKTAG_TRANSACTION 共享锁。

**9. 为什么死锁检测判"无死锁"后该进程不再重复检测也不会漏报？**

答：归纳论证（8.8.1/8.8.3）：死锁环的成立需要最后一条等待边加入，而加边者是一个新开始等待的进程，它挂起前必武装自己的 deadlock_timeout 定时器；该定时器到期在环成立之后，检测时必见完整环。故"每个环至少被其最后入环者检测一次"。已检测过的进程的等待关系不再变化（它睡着），其旧结论保持有效。这让检测成本严格绑定"长等待"事件而非锁等待总量——配合检测需冻结全部 16 个锁分区的高成本，构成懒触发的完整理由；InnoDB 8.0 提供 innodb_deadlock_detect=OFF 是同一权衡的反向印证。

**10. 用转账例子（A=100,B=100，约束 A+B≥0，各扣 150）说明 write skew，并解释 SSI 为什么允许假阳性、错误信息里的 "pivot" 指什么。**

答：T1 验 sum=200 后扣 A，T2 验 sum=200 后扣 B：改的是不同行，行锁/EPQ/并发更新检查全不触发，RC/RR 下双双提交得 A+B=-100。依赖分析：T1 读了 T2 将写的 B 行旧版 ⇒ T1--rw-->T2；对称 T2--rw-->T1，成环。每个事务都有一进一出两条 rw 边，即危险结构 Tin--rw-->Tpivot--rw-->Tout 中的 pivot（predicate.c:4446-4462 注释）；SSI 在建边（FlagRWConflict→OnConflict_CheckForSerializationFailure）或提交（PreCommit_...）时发现 pivot 且 Tout 先提交的条件成立，杀 writer/给 DOOMED/杀 reader（视谁可杀，predicate.c:4580-4612），报 40001。允许假阳性：SSI 只追踪 rw 边不追踪 wr/ww，危险结构未必真闭合成环——完整环检测的追踪成本远高于偶尔误杀一个可重试事务；实践中假阳性率低，应用侧统一"遇 40001 重试"即可。

---

## 附录：本章走读函数索引（行号按 master/20devel 核实）

| 函数 / 结构 | 位置 | 走读节 |
|---|---|---|
| TransState / TBlockState / TransactionStateData | xact.c:143 / 159 / 195 | 8.1.1 / 8.1.4 |
| StartTransaction / CommitTransaction / AbortTransaction | xact.c:2106 / 2270 / 2855 | 8.1.2 |
| AssignTransactionId / TransactionIdIsCurrentTransactionId | xact.c:637 / 943 | 8.1.3 / 8.4.2 |
| GetNewTransactionId | varsup.c:68（发布段 246-256） | 8.2.2 |
| TransactionIdPrecedes | transam.h:263 | 8.2.3 |
| SnapshotData | snapshot.h:138 | 8.3.1 |
| GetSnapshotData / GetSnapshotDataReuse | procarray.c:2114 / 2034 | 8.3.3 / 8.3.4 |
| ProcArrayEndTransaction（组退出 686） | procarray.c:663 | 8.1.2 |
| SetHintBitsExt / SetHintBits | heapam_visibility.c:142 / 199 | 8.4.1 |
| HeapTupleSatisfiesMVCC | heapam_visibility.c:939（主体 956-1096） | 8.4.2 |
| 可见性族：Self / Toast / Update / Dirty / Vacuum / Visibility | heapam_visibility.c:297 / 452 / 511 / 759 / 1113 / 1732 | 8.4.5 |
| XidInMVCCSnapshot | snapmgr.c:1868 | 8.4.3 |
| TransactionLogFetch / SubTrans{SetParent,GetTopmostTransaction} | transam.c:52 / subtrans.c:92,170 | 8.4.4 / 8.1.3 |
| TransactionIdSetTreeStatus / TransactionIdSetStatusBit | clog.c:192 / 669 | 8.5.1 |
| x86_64 tas() | s_lock.h:196-226 | 8.6.1 |
| LWLock 状态位定义 | lwlock.c:96-108 | 8.6.2 |
| LWLockAttemptLock / LWLockAcquire / LWLockRelease / LWLockWakeup | lwlock.c:764 / 1150 / 1767 / 904 | 8.6.2 |
| LWLockQueueSelf / LWLockDequeueSelf | lwlock.c:1018 / 1061 | 8.6.2 |
| LockConflicts 矩阵 / EligibleForRelationFastPath | lock.c:68 / 270 | 8.6.3 |
| LockAcquireExtended | lock.c:833（fastpath 984-1055，共享表 1062-1122） | 8.6.3 |
| FastPathGrantRelationLock / FastPathTransferRelationLocks | lock.c:2790 / 2869 | 8.6.3 |
| JoinWaitQueue / WaitOnLock / ProcSleep / CheckDeadLock | proc.c:1209 / lock.c:1940 / proc.c:1378 / 1887 | 8.6.4 |
| DeadLockCheck / DeadLockCheckRecurse / TestConfiguration | deadlock.c:220 / 312 / 378 | 8.8.2 |
| FindLockCycle(Recurse/Member) | deadlock.c:446 / 457 / 536 | 8.8.2 |
| 行锁模式 / infomask 位 | lockoptions.h:53-59 / htup_details.h:194-209 | 8.7.2 |
| MultiXactIdCreate(FromMembers) / GetMultiXactIdMembers | multixact.c:358 / 715 / 1172 | 8.7.3 |
| heap_lock_tuple / heapam_tuple_lock | heapam.c:4728 / heapam_handler.c:265 | 8.7.4 |
| PredicateLock{Relation,Page,TID} | predicate.c:2505 / 2528 / 2550 | 8.9.3 |
| GetSerializableTransactionSnapshot（备库拒绝 1624） | predicate.c:1611 | 8.3.6 / 8.9.7 |
| CheckForSerializableConflictOut / In | predicate.c:3952 / 4265 | 8.9.4 / 8.9.5 |
| OnConflict_CheckForSerializationFailure | predicate.c:4465 | 8.9.6 |
| PreCommit_CheckForSerializationFailure | predicate.c:4632 | 8.9.6 |

---

## 动手实验建议

1. **版本与锁的可视化**：开两个 psql，会话 A `BEGIN; UPDATE t SET ...` 不提交；会话 B 观察 `pg_locks`（注意 fastpath 列、granted=false 的 transactionid 锁、一闪而过的 tuple 锁）与 `SELECT xmin, xmax, ctid FROM t` 的版本差异；装 pageinspect 扩展用 `heap_page_items(get_raw_page('t',0))` 直接看元组头的 infomask 位，对照 8.4.1/8.7.2 的位定义解码；
2. **hint bit 写放大**：`CREATE TABLE big AS SELECT ...` 灌一千万行后 `CHECKPOINT`，第一次 `SELECT count(*)` 时用 `EXPLAIN (ANALYZE, BUFFERS)` 看 dirtied 页数；第二次扫描 dirtied 归零——8.4.1 的现场；
3. **suboverflow 悬崖**：写个 DO 块开 70 个带 INSERT 的 SAVEPOINT 挂住不提交，另一会话跑基准查询，对比 `pg_stat_slru` 中 subtrans 行的 blks_read 前后差（8.4.6）；
4. **gdb 走读**：attach 到 backend，在 `HeapTupleSatisfiesMVCC`（heapam_visibility.c:939）与 `GetSnapshotData`（procarray.c:2114）下断点，单步走一次"拍快照 → 判可见"；对照 8.4.2 的分支标号验证你构造的场景落进哪个分支；
5. **死锁时序**：按 8.8.3 的时序表操作两个会话，把 `deadlock_timeout` 调到 100ms 观察检测提前，或调到 30s 观察双方长挂 1 分钟才裁决；
6. **SSI 全套**：两个 SERIALIZABLE 会话复现 8.9.1 的转账 write skew，观察 40001 错误细节文本里的 "pivot" 字样，且 `pg_stat_database.deadlocks` 不增加（SSI 回滚不是死锁）；按 8.9.2 的三事务脚本复现只读事务异常；再把只读会话换成 `SERIALIZABLE READ ONLY DEFERRABLE`，观察它等待安全快照的行为。

## 延伸阅读

- **源码内文档**（比任何书都新、都准）：`src/backend/access/transam/README`（事务系统总纲，本章 I-3/I-4 不变式的原始论证）、`src/backend/storage/lmgr/README`（锁体系与死锁算法，软边/拓扑排序的完整推导）、`src/backend/storage/lmgr/README-SSI`（SSI 理论到实现的映射，含只读优化的证明）、`src/backend/storage/lmgr/README.barrier`（内存屏障使用规范，8.2.2/8.3.3 屏障配对的依据）；
- **论文**：Cahill, Röhm & Fekete, "Serializable Isolation for Snapshot Databases" (SIGMOD 2008)——SSI 原始论文；Ports & Grittner, "Serializable Snapshot Isolation in PostgreSQL" (VLDB 2012)——PG 实现者自述，比论文更贴近 predicate.c；Fekete et al., "Making Snapshot Isolation Serializable" (TODS 2005)——危险结构定理出处；
- **本系列关联章**：第 5 章（提交路径与 WAL 的交接）、第 9 章（崩溃恢复如何重建 CLOG 一致性）、第 10 章（VACUUM——本章所有"延后付账"的总收银台）、第 13 章（热备快照与 KnownAssignedXids 的另一半故事）。

---

*本章完。下一章（09-WAL 与崩溃恢复）将回答本章多处埋下的伏笔：commit 记录长什么样、CLOG 如何在恢复中重建、FPI 为什么能修复 torn page。*
