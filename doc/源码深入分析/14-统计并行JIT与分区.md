...
`GatherMerge`（nodeGatherMerge.c，PG 10，commit 355d3993c5）解决"子路径有序、汇聚后仍要有序"：每 worker 产出有序流，leader 用二叉堆做 N+1 路归并（`binaryheap_allocate(nreaders + 1, ...)`，nodeGatherMerge.c:430，+1 是 leader 自己的本地流）。核心 `gather_merge_getnext()`（nodeGatherMerge.c:547）：
...
教科书归并的两个工程化改造：其一，堆里存的不是元组而是**参与者编号**（0 = leader 本地流，1..N = worker），比较函数按各自槽位里的当前元组比 sort key——避免元组在堆中搬家；其二，`replace_first` 而非"弹出再插入"——堆顶换值后单次下滤，省一半比较。注意与 Gather 的关键差异：GatherMerge **必须阻塞**（`gather_merge_readnext(..., nowait=false)`）——最小元组可能在任何一路，任何一路暂时无数据都不能先吐别路的元组，否则破坏全序。为了缓解逐条阻塞读的开销，`load_tuple_array()`（nodeGatherMerge.c:597）在非阻塞模式下顺手把队列里已有的元组一次搬 `MAX_TUPLE_STORE`（10）条进本地缓冲——摊薄 shm_mq 读协议的固定成本。它使 `Sort` 可以下推到 Gather 之下并行执行——与 Spark 的 sort-merge 落盘归并、Kafka 多分区按时间戳归并同构。
...
**发送主线**（`shm_mq_send` shm_mq.c:331 → `shm_mq_sendv` shm_mq.c:363 → `shm_mq_send_bytes` shm_mq.c:916）：先发 8 字节长度字，再发 payload（消息语义架在字节流上）。`shm_mq_send_bytes` 的循环骨架：
...
三分支解说：① 可用空间的计算里 `wb` 要**加上本地 pending**——共享计数落后于真实写入量，不加会把自己没冲刷的数据当空闲空间覆盖掉；② 队列满时才冲刷 pending 并 `SetLatch` 叫醒 receiver（平时不叫，见批量推进优化），nowait 模式返回 `SHM_MQ_WOULD_BLOCK`（Gather 的非阻塞轮询依赖这个返回值），阻塞模式 `WaitLatch` 睡到 receiver 腾出空间；③ 有空间则 `memcpy` 一次写到环尾为止（回绕留给下一轮），写入量按 MAXALIGN 对齐——环大小是 MAXIMUM_ALIGNOF 的倍数，保证长度字永远不会被劈在环尾（shm_mq.c:1050-1055 注释）。开头的 `pg_compiler_barrier()`（shm_mq.c:947）防止编译器把 `mq_detached` 缓存进寄存器跨迭代复用——对端随时可能 detach。

**接收主线**（`shm_mq_receive` shm_mq.c:574 → `shm_mq_receive_bytes` shm_mq.c:1081）对称。一个精妙细节：消息若在环中连续，**直接返回环内指针**不拷贝（shm_mq.c:106-111 注释），只有回绕的消息才拼接到本地缓冲——leader 消费元组通常是零拷贝的。另一个细节（shm_mq.c:1130-1143）：发现 `mq_detached` 后必须**先读屏障再重读一次 written**——sender 可能"推进计数、然后 detach"，直接返回 DETACHED 会丢掉最后一批数据。
...
2. **`get_partition_for_tuple()`**（execPartition.c:1570）按策略分派。hash 分支最短：
...
```
   ...
   list/range 走二分，且**带命中缓存**（execPartition.c:1579-1588 注释）。以 list 为例：
...
```
   ...
   同一分区连续命中超过阈值后，先用上次命中的边界做**一次**等值比较；批量导入按时间有序到来时几乎每行命中上一行的分区，O(log n) 二分降为 O(1)。hash 不缓存（取模本来就 O(1)）；NULL 值不进二分，list 直接走 `null_index`（NULL 与任何值的比较都不是"相等"，必须旁路）；
